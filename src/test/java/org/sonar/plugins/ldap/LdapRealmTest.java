...
public class LdapRealmTest {
  ...
  public void normal() {
	...
	realm.init();
	assertThat(realm.getLoginPasswordAuthenticator()).isInstanceOf(
		LoginPasswordAuthenticator.class).isInstanceOf(LdapAuthenticator.class);
	assertThat(realm.getUsersProvider()).isInstanceOf(
		ExternalUsersProvider.class).isInstanceOf(LdapUsersProvider.class);
	assertThat(realm.getGroupsProvider()).isNull();
  }

  @Test
  public void noConnection() {
	Settings settings = new Settings().setProperty("ldap.url",
		"ldap://no-such-host").setProperty("ldap.group.baseDn",
	...
	assertThat(realm.getUsersProvider()).isInstanceOf(
		ExternalUsersProvider.class).isInstanceOf(LdapUsersProvider.class);
	assertThat(realm.getGroupsProvider()).isInstanceOf(
		ExternalGroupsProvider.class).isInstanceOf(LdapGroupsProvider.class);
