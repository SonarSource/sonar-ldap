...
public class LdapGroupMapping {
  ...
  public LdapGroupMapping(Settings settings, String ldapIndex) {
	...
	if (StringUtils.isNotBlank(objectClass)
	  ...
	  objectClass = StringUtils
		  .defaultString(objectClass, DEFAULT_OBJECT_CLASS);
	  ...
	  LoggerFactory
		  ...
		  .warn(
			  "Properties 'ldap.group.objectClass' and 'ldap.group.memberAttribute' are deprecated"
				  + " and should be replaced by single property 'ldap.group.request' with value: "
				  + req);
	} else {
	  req = StringUtils.defaultString(
		  settings.getString(ldapIndex + ".group.request"), DEFAULT_REQUEST);
	}
