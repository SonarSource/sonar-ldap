...
public class LdapAuthenticator implements LoginPasswordAuthenticator {
  ...
  public boolean authenticate(String login, String password) {
	for (String ldapIndex : userMappings.keySet()) {
	  ...
	  if (contextFactories.get(ldapIndex).isSasl()) {
		...
		try {
		  result = userMappings.get(ldapIndex)
			  .createSearch(contextFactories.get(ldapIndex), login)
			  .findUnique();
  ...
  private boolean checkPasswordUsingBind(String principal, String password,
	...
	try {
	  context = contextFactories.get(ldapIndex).createUserContext(principal,
		  password);
	  return true;
	} catch (NamingException e) {
	  LOG.debug("Password not valid for user {} in " + ldapIndex + ": {}",
  ...
  private boolean checkPasswordUsingGssapi(String principal, String password,
	...
	LoginContext lc;
	try {
	  lc = new LoginContext(getClass().getName(), new CallbackHandlerImpl(
		  principal, password));
	  lc.login();
